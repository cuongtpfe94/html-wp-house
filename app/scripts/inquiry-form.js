...
(function () {
  ...
  const formData = {
    ...
    iso: {
      ...
      categories: [
        { value: 'certification', text: '認証取得' },
        { value: 'registration', text: '認証登録組織' },
        { value: 'iso-audit', text: 'ISO/IEC 27001：2022 規格改訂' },
        { value: 'other-iso', text: 'その他' },
      ],
      subcategories: [],
    },
    research: {
    ...
    other: {
      ...
      sectionTitle: 'その他',
      categories: [
        { value: 'blf-advisor', text: 'BLRアドバイザー登録制度' },
        { value: 'rem-quality', text: 'リフォーム業務品質審査登録制度' },
        { value: 'hld-service', text: '住宅履歴情報管理支援サービス' },
        { value: 'pr-homepage', text: '広報・ホームページ' },
        { value: 'other-inquiry', text: 'その他、当財団に関するお問い合わせ' },
      ],
      subcategories: [],
    },
  };
  ...
  const colorClasses = {
    red: { border: '#e68082', bg: '#f9dfe0' },
    blue: { border: '#4987B5', bg: '#D3E5F1' },
    orange: { border: '#F0962B', bg: '#FBE5CA' },
    teal: { border: '#68BB61', bg: '#D9EED7' },
    purple: { border: '#5AB4B4', bg: '#D6ECEC' },
    green: { border: '#7F7F7F', bg: '#DFDFDF' },
    gray: { border: '#7F7F7F', bg: '#DFDFDF' },
  };
  ...
  function updateFormContent(categoryKey) {
    ...
    const categoryGrid = categoryField
      ? categoryField.querySelector('.hpd-inquiry__category-grid')
      : null;
    ...
    const companyField = document
      .querySelector('label[for="company"]')
      ?.closest('.hpd-inquiry__field');
    const departmentField = document
      .querySelector('label[for="department"]')
      ?.closest('.hpd-inquiry__field');
  ...
  function toggleCategoryRecruitFields(recruitCategoryValue) {
    const resumeField = document
      .querySelector('label[for="resume"]')
      ?.closest('.hpd-inquiry__field');
    const entrySheetField = document
      .querySelector('label[for="entry-sheet"]')
      ?.closest('.hpd-inquiry__field');

    const dateField = document
      .querySelector('label[for="date"]')
      ?.closest('.hpd-inquiry__field');

    const internshipField = document
      .querySelector('label[for="internship"]')
      ?.closest('.hpd-inquiry__field');
    const internshipSurveyField = document
      .querySelector('label[for="internship-survey"]')
      ?.closest('.hpd-inquiry__field');
  ...
  function setupRecruitCategoryListener() {
    const recruitCategoryRadios = document.querySelectorAll(
      'input[name="inquiry-category-recruit"]'
    );
    ...
    const checkedRecruitRadio = document.querySelector(
      'input[name="inquiry-category-recruit"]:checked'
    );
  ...
  function toggleAddressRequired(categoryKey) {
    const addressField = document
      .querySelector('label[for="address"]')
      ?.closest('.hpd-inquiry__field');
    if (!addressField) return;

    const addressInput = document.querySelector('#address');
    const addressRequired = addressField.querySelector(
    ...
    if (categoryKey === 'recruit') {
      const recruitCategoryRadio = document.querySelector(
        'input[name="inquiry-category-recruit"]:checked'
      );

      const recruitCategoryValue = recruitCategoryRadio
        ? recruitCategoryRadio.value
        : null;
      const requiredValues = ['entry', 'one-day', 'internship'];

      if (
        recruitCategoryValue &&
        requiredValues.includes(recruitCategoryValue)
      ) {
  ...
  function togglePhoneRequired(categoryKey) {
    ...
    const phoneFieldContainer = document
      .querySelector('.hpd-inquiry__phone-group')
      ?.closest('.hpd-inquiry__field');
